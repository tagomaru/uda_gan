...
class _netD(nn.Module):
    def __init__(self, opt, nclasses_l):
        super(_netD, self).__init__()
        
        self.ndf = opt.ndf
        self.feature = nn.Sequential(
            nn.Conv2d(4, self.ndf, 3, 1, 1),            
            ...
            nn.Conv2d(self.ndf*4, self.ndf*2, 3, 1, 0),           
            nn.BatchNorm2d(self.ndf*2),
            nn.LeakyReLU(0.2, inplace=True),
            nn.MaxPool2d(4,4)           
        )

        self.classifier_cl = nn.Sequential(nn.Linear(self.ndf*2, nclasses_l))  
        # self.classifier_cq = nn.Sequential(nn.Linear(self.ndf*2, nclasses_q))              
    ...
    def forward(self, input):       
        ...
        output_cl = self.classifier_cl(output.view(-1, self.ndf*2))
        # output_cq = self.classifier_cq(output.view(-1, self.ndf*2))
        return output_s, output_cl#, output_cq
...
class _netF(nn.Module):
    def __init__(self, opt):
        ...
        self.ndf = opt.ndf
        self.feature = nn.Sequential(
            nn.Conv2d(4, self.ndf, 5, 1, 1),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(2, 2),
            
            nn.Conv2d(self.ndf, self.ndf, 5, 1, 1),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(2, 2),
